...
/**
 * Precompiled Pertama.java-application-conventions script plugin.
 **/
public class PertamaJavaApplicationConventionsPlugin implements org.gradle.api.Plugin<org.gradle.api.internal.project.ProjectInternal> {
    ...
    public void apply(org.gradle.api.internal.project.ProjectInternal target) {
        assertSupportedByCurrentGradleVersion();
                target.getPluginManager().apply("Pertama.java-common-conventions");
        target.getPluginManager().apply("application");

        try {
            Class<? extends BasicScript> precompiledScriptClass = Class.forName("precompiled_PertamaJavaApplicationConventions").asSubclass(BasicScript.class);
